...
const LeadsDashboard = () => {
  ...
  const loadAllLeads = async () => {
    ...
    try {
      ...
      const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/leads`, {
  ...
  const markAsRead = async (leadId: string) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/leads/${leadId}/read`, {}, {
        headers: { Authorization: `Bearer ${token}` },
      });

      setLeads(leads.map((lead) => (lead._id === leadId ? { ...lead, isRead: true } : lead)));
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
        {/* Header */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 sm:gap-0">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Leads Dashboard</h1>
            <p className="text-gray-600 mt-1 text-sm sm:text-base">Manage customer inquiries and messages</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={loadAllLeads}
              disabled={loading}
              className="flex items-center gap-2 px-3 sm:px-4 py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors"
            >
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
          ...
          <div className="p-3 sm:p-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
            ...
            {selectedLeads.length > 0 && (
              <div className="flex items-center gap-2 mt-2 sm:mt-0 ml-auto">
                <span className="text-sm sm:text-base text-gray-600">{selectedLeads.length} selected</span>
          ...
          <div className="divide-y divide-gray-200">
            {loading ? (
              ...
              paginatedLeads.map((lead) => (
                <OpenMail key={lead._id} lead={lead} onDelete={deleteLead}>
                  <div
                    className={`p-3 sm:p-4 hover:bg-gray-50 cursor-pointer transition-colors flex flex-col sm:flex-row sm:items-center gap-3 ${
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-4">
                        <div className="truncate">
                          ...
                          <p className="text-xs sm:text-sm text-gray-500 truncate mt-1">{lead.subject}</p>
                          <p className="text-xs sm:text-sm text-gray-400 truncate">{lead.content}</p>
                        </div>
                        <div className="flex items-center gap-3 text-xs sm:text-sm text-gray-500 mt-1 sm:mt-0">
                          <span className="flex items-center gap-1">
                            <Clock className="w-3 h-3" />
                            {formatDate(lead.createdAt)}
                          </span>
                          {!lead.isRead && <div className="w-2 h-2 bg-blue-600 rounded-full"></div>}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {/* Pagination */}
          {totalPages > 1 && (
            <div className="p-3 sm:p-4 border-t border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 text-sm sm:text-base">
              <span>
