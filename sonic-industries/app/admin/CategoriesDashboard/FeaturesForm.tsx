...
const FeaturesForm = ({ formData, setFormData }: FeaturesFormProps) => {
  ...
  const isValidIcon = (iconName: string): boolean => {
    ...
    try {
      ...
      return typeof IconComponent === "function" || (typeof IconComponent === "object" && IconComponent !== null);
    } catch {
      return false;
    }
  };
          ...
          <div className="space-y-3">
                ...
                {(() => {
                  ...
                  const filteredIcons = Object.entries(LucideIcons)
                    .filter(([name, IconComponent]) => {
                      ...
                      const invalidNames = ['createLucideIcon', 'default', 'LucideIcon'];
                      if (invalidNames.some(invalid => name.includes(invalid))) return false;
                    ...
                    try {
                      ...
                      return (
                        <button
                          ...
                          onClick={() =>
                            setNewFeature((prev) => ({ ...prev, image: name }))
                          }
                        >
                      ...
                      console.warn(`Failed to render icon button for: ${name}`, error);
                      return null;
                    }
                  }).filter(Boolean);
                })()}
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={!newFeature.name?.trim() || !isValidIcon(newFeature.image || "")}
